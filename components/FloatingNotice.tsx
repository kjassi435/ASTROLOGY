...
export function FloatingNotice({ data, embedded = false }: { data: Record<string, string>; embedded?: boolean }) {
  ...
  const CardInner = (
    <>
      ...
      <div className="p-3">
        {cur.title ? (
          <div className="text-[13.5px] font-bold leading-tight text-slate-900 flex items-start gap-1.5">
            <span className="flex-1">{cur.title}</span>
            {cur.link ? <IconExternal size={12} className="mt-0.5 shrink-0 text-primary" /> : null}
          </div>
        ) : null}
        {cur.desc ? <p className="mt-1 text-[11.5px] leading-snug text-slate-500 line-clamp-2">{cur.desc}</p> : null}
          ...
          <div className="flex items-center gap-1.5" role="tablist" aria-label="Notice slides">
            {slides.map((_, i) => (
              <button
                ...
                onClick={() => setIdx(i)}
                className={i === idx ? "h-1.5 w-4 rounded-full bg-primary transition-all" : "h-1.5 w-1.5 rounded-full bg-slate-200 transition-all hover:bg-slate-300"}
              />
            ))}
          </div>
          {slides.length > 1 ? (
            <div className="flex items-center gap-1">
              <button aria-label="Previous slide" onClick={() => go(idx - 1)} className="h-6 w-6 rounded-full border border-slate-200 bg-white flex items-center justify-center text-slate-500 hover:bg-slate-50">
                <IconChevronLeft size={11} />
              </button>
              <button aria-label="Next slide" onClick={() => go(idx + 1)} className="h-6 w-6 rounded-full border border-slate-200 bg-white flex items-center justify-center text-slate-500 hover:bg-slate-50">
                <IconChevronLeft size={11} className="rotate-180" />
              </button>
            </div>
          ) : null}
        </div>
  ...
  if (embedded) {
    return (
      <div className="relative w-full" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
        <div className="relative bg-white rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.25)] border border-slate-200 overflow-hidden">
          <button
            aria-label="Close notice"
            onClick={handleClose}
            className="absolute top-2 right-2 z-10 h-7 w-7 rounded-full bg-white border border-slate-200 shadow flex items-center justify-center text-slate-600 hover:bg-slate-50"
          >
            <IconClose size={13} />
          </button>
          {cur.link ? (
